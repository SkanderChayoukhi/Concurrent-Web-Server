...
import threading
import requests
import random  # Pour générer des délais aléatoires
import matplotlib.pyplot as plt
import argparse

# Function to send a request to the server with a random delay
def send_request(url):
    try:
        delay = random.uniform(0.1, 0.5)  # Génère un délai aléatoire entre 1 et 5 secondes
        time.sleep(delay)  # Applique le délai avant d'envoyer la requête
