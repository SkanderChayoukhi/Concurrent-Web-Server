...
//modified the accept_or_die function to handle the EINTR error while shuting down
#define accept_or_die(s, addr, addrlen) \
    ({ int rc = -1; \
       if (s != -1) {  /* Only attempt accept if socket is valid */ \
           rc = accept(s, addr, addrlen); \
           if (rc == -1) { \
               if (errno == EINTR) { \
                   rc = -1;  /* Ignore EINTR, just return -1 and continue the loop */ \
               } else if (errno == EBADF) { \
                   rc = -1;  /* Accept failed because socket was closed */ \
               } else { \
                   perror("accept"); \
               } \
           } \
